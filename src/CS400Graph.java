...
// TA: Brianna Cochran
// Lecturer: Gary Dahl
// Notes to Grader: requires GraphADT.java
...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		if (!this.containsVertex(start) || !this.containsVertex(end) || start == null || end == null)
		...
		PriorityQueue<Path> pathsPQ = new PriorityQueue<Path>();
		Path current = null;
		Path clone;
		Vertex startV = this.vertices.get(start);
		pathsPQ.add(new Path(startV));
		visited.put(start, startV);
		while (!pathsPQ.isEmpty()) {
			current = pathsPQ.poll();
			if (current.end.data.equals(end) && current.start.data.equals(start)) {
				return current;
			}
			for (Edge next: current.end.edgesLeaving) {
				clone = new Path(current);
				if (!visited.containsValue(next.target)) {
					clone.extend(next);
					pathsPQ.add(clone);
					visited.put(next.target.data, next.target);
				}
			}
		}
